...
@section('titre', 'Catégories des sorties')
...
<style>
    ...
    .btn-new {
        background: var(--forest); color: #fff; text-decoration: none; padding: .55rem 1.1rem;
        border-radius: 4px; font-size: .85rem; font-weight: 500; display: inline-flex; align-items: center; gap: .4rem;
    }
...
<div class="page-head">
    <div>
        <h1>Catégories des sorties</h1>
        <p>Gérez les différentes catégories de dépenses de la caisse.</p>
    </div>
    <a href="{{ route('categories-sorties.create') }}" class="btn-new">
...
<div class="cs-panel">
    ...
    @if(($categories ?? collect())->isEmpty())
        <div class="cs-empty">
            ...
            <p class="sub">Commencez par créer votre première catégorie de sortie.</p>
            <a href="{{ route('categories-sorties.create') }}" class="btn-new">
        ...
        <table class="cs-table" id="tableCategories">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="actions">
                        <a href="{{ route('categories-sorties.edit', $categorie->id) }}" class="edit"><i class="bi bi-pencil"></i></a>
                        <form action="{{ route('categories-sorties.destroy', $categorie->id) }}" method="POST" style="display:inline" onsubmit="return confirm('Supprimer cette catégorie ?');">
                            @csrf @method('DELETE')

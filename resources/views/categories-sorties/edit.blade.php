...
<div class="page-head">
    ...
    <p>Mettre à jour ce type de dépense.</p>
</div>

<div class="form-card">
    <form action="{{ route('categories-sorties.update', $categorie->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-row">
            <label>Libellé</label>
        ...
        <div class="form-row">
            ...
            <textarea name="description" rows="4">{{ old('description', $categorie->description) }}</textarea>
        </div>
        <div class="form-actions">
            <button type="submit" class="btn-primary"><i class="bi bi-check-lg"></i> Enregistrer les modifications</button>
            <a href="{{ route('categories-sorties.index') }}" class="btn-cancel">Annuler</a>
        </div>
    </form>
</div>
@endsection